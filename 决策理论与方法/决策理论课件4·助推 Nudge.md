---
title: "决策理论课件4·助推 Nudge"
course: "决策理论与方法"
description: "石婧老师双语课件《助推》：Thaler 与 Sunstein 的自由主义温和专制主义、选择架构与双系统思维，五种可预测的认知偏差（锚定、安于现状、可得性、代表性、不切实际的乐观）及公共管理实验证据，以及 iNcentives、Mappings、Defaults、Feedback、Expect Error 等良好选择架构原则。"
...
| Automatic System（直觉系统） | Reflective System（反思系统） |
| --- | --- |
| Fast（快） | Slow（慢） |
| Unconscious（无意识） | Conscious（有意识） |
| Effortless（不费力） | Effortful（费力） |
| Doer（行动者） | Planner（计划者） |
| Uncontrolled（不受控） | Controlled（受控） |
...
<!--col:M-->
#### 02 · 实验设计
实验 6：同一道决策题，只替换一个数字锚。

样本与分组：600 名意大利公共部门员工与管理者；调查时间：2016 年 6—7 月，共 5 轮。实验 6 有效样本：545；低锚组 228 人，高锚组 317 人。注：第一波低锚情境文字有误，作者剔除了 55 份相关回答。

决策情境：你是某中型市政府公共关系办公室的高级经理，需要决定下属回复公民电子邮件的最长工作日数。随机分配后，先回答一个带数字锚的问题。

- 低锚组（2）：上限应高于还是低于 2 个工作日？
- 高锚组（90）：上限应高于还是低于 90 个工作日？

随后，两组回答同一问题：最长回复期限应设为多少个工作日？

资料来源：Bellé, Cantarelli & Belardinelli（2018），Table 1（p. 832）与 Table 2（p. 833）。
...
<!--col:M-->
#### 03 · 结果与启示
结果：高锚把建议的回复期限抬高约 68.5%。

组间差异 +21.8 天 = 高锚组均值 − 低锚组均值。

- 低锚组：31.82 天（N = 228；SD = 54.81）
- 高锚组：53.62 天（N = 317；SD = 53.07）

差异显著：p 值 < .001。图 3：按锚点分组的最长回复期限均值；误差线为 95% CI。

管理启示：制定服务时限、绩效标准或预算基准时，先看到的数字可能改变标准本身，制度设计需要意识到这种系统性偏差。
...
经典证据：CD 播放器实验中，增加一个高端型号后，推迟购买的比例从 34% 升至 46%。谢林的例子：面对两套百科全书难以取舍，最终一套也没买；若只有一套，他本会购买。

资料来源：Bellé, Cantarelli & Belardinelli（2018），Status Quo 部分，p. 831；Tversky & Shafir（1992）。
...
<!--col:M-->
#### 02 · 实验设计
实验 8–9：把可替代方案从一个增加到两个。

共同设置：600 名意大利公共部门员工与管理者。实验 8：一个替代承包商组 235 人；两个替代承包商组 263 人。实验 9：一个修复方案组 155 人；两个修复方案组 141 人。

实验 8 · 是否续约现有软硬件承包商：现有承包商之外，市场分析发现一个或两个可行替代者；更换承包商需要时间和文书工作。一个替代方案：Omega，服务略好，成本相同。两个替代方案：Omega 同上；Sigma，服务相当，成本略低。

实验 9 · 是否直接发布新的招标：工资软件故障，现任承包商多次未能修复；招标发布前，IT 部门提出一个或两个可能有效的修复程序。一个修复程序：尝试可能耗时并造成延误。两个修复程序：选择更多，但行动冲突也更强。
...
核心发现：
- 1.78×：实验 8，两个替代承包商——选择续约现有承包商的发生比，是一个替代方案组的 1.78 倍。
- 5.74×：实验 9，两个修复方案——仍直接发布招标、不尝试修复的发生比，是一个修复方案组的 5.74 倍。

（实验 8：继续选择现有承包商，横轴替代承包商数量、纵轴选择现状比例；实验 9：不尝试修复而直接招标，横轴可用修复程序数量、纵轴维持原定招标行动比例；误差线为 95% CI。）

管理启示：增加备选方案并不必然促进更换供应商或采取行动；方案数量本身也可能诱发拖延与维持现状。
